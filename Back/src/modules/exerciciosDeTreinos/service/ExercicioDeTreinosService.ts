...
export class ExercicioDeTreinoService {
  ...
  async create(data: z.infer<typeof ExercicioDeTreinoSchema>) {
    ...
    const exercicioExistente = await this.exercicioRepository.findById(data.exercicio_id);
    if (!exercicioExistente) {
      throw new AppError("Exercício não encontrado", 404);
    }

    const existingRelation = await this.exercicioDeTreinoRepository.findByTreinoId(data.treino_id);

    if (existingRelation != null && existingRelation.exercicio_id == data.exercicio_id) {
  ...
  async update(exercicio_treino_Id: number, data: z.infer<typeof ExercicioDeTreinoSchema>) {
    const relation = await this.exercicioDeTreinoRepository.findById(exercicio_treino_Id);

    if (!relation) {
      throw new AppError("Relacionamento entre treino e exercício não encontrado", 404);
    }

    return await this.exercicioDeTreinoRepository.update(exercicio_treino_Id, data);
  }
  ...
  async findByTreinoId(treino_id: number) {
    const relation = await this.exercicioDeTreinoRepository.findByTreinoId(treino_id);
